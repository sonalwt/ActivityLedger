# ActivityWatch Sync Script - Fixed Version with Dynamic Credentials
param(
...
# Function to get configuration (reuse from sync_dynamic.ps1)
function Get-SyncConfiguration {
    ...
    if (-not $config.DeveloperName -or -not $config.ApiToken) {
        try {
            $regName = (Get-ItemProperty -Path "HKCU:\Software\TimesheetSync" -Name "DeveloperName" -ErrorAction Stop).DeveloperName
            if ($regName -and -not $config.DeveloperName) {
                $config.DeveloperName = $regName
                Write-Host "Using developer name from secure storage: $regName" -ForegroundColor Green
            }
        } catch {}
        
        $tokenPath = "$env:APPDATA\TimesheetSync\token.secure"
        if ((Test-Path $tokenPath) -and -not $config.ApiToken) {
            try {
                ...
                Write-Host "Using API token from secure storage" -ForegroundColor Green
            } catch {}
        }
    }
...
Write-Host "========================================"
Write-Host "ActivityWatch Sync - Enhanced Version"
Write-Host "========================================"
...
if (-not $WORKING_SERVER_URL) {
    ...
    Write-Host ""
    Write-Host "Run diagnose_connection.bat for detailed diagnostics"
    exit 1
}
